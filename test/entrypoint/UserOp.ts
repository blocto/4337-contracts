...
export async function fillAndSignWithCoSigner (op: Partial<UserOperation>, signer: Wallet | Signer, cosigner: Wallet | Signer, entryPoint?: EntryPoint, getNonceFunction = 'getNonce'): Promise<UserOperation> {
  const provider = entryPoint?.provider
  ...
  const signerSignature = await signer.signMessage(message)
