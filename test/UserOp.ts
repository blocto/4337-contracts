...
import { Bytes, joinSignature, concat } from '@ethersproject/bytes'
...
// following for Blocto Wallet sign EIP-191 V0
async function signMessageEIP191V0 (wallet: Wallet, address: string, message: Bytes | string): Promise<string> {
  return joinSignature(wallet._signingKey().signDigest(hashMessageEIP191V0(address, message)))
}
...
export async function fillAndSignWithCoSigner (op: Partial<UserOperation>, signer: Wallet, cosigner: Wallet, entryPoint?: EntryPoint, getNonceFunction = 'getNonce'): Promise<UserOperation> {
  const provider = entryPoint?.provider
  ...
  const signerSignature = await signMessageEIP191V0(signer, op.sender, message)
  const cosignerSignature = await signMessageEIP191V0(cosigner, op.sender, message)
