...
import {
  ...
  BloctoAccount__factory, BloctoAccountFactory
} from '../typechain'

import { EntryPoint, EntryPoint__factory, IEntryPoint } from '@account-abstraction/contracts'
...
export async function createAccount (
  ...
  const accountFactory = _factory ?? await new BloctoAccountFactory__factory(ethersSigner).deploy(entryPoint)
  const implementation = await accountFactory.accountImplementation()
  await accountFactory.createAccount(authorizedAddresses, cosignerAddresses, recoverAddresses, 0)
  const accountAddress = await accountFactory.getAddress(authorizedAddresses, cosignerAddresses, recoverAddresses, 0)
  const proxy = BloctoAccount__factory.connect(accountAddress, ethersSigner)
  return {
    implementation,
    accountFactory,
    proxy
